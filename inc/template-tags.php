...
/**
 * Renders a custom 'secondary logo' image, if set
 *
 * @return void
 */
function rhd_secondary_logo() {
	$options = get_option( 'rhdwp_options' );
	
	if ( isset( $options['secondary_logo'] ) && absint( $options['secondary_logo'] ) ) {
		printf( '<a href="%s" class="custom-logo-link" rel="home">%s</a>', home_url( '/' ), wp_get_attachment_image( $options['secondary_logo'], 'medium', false, ['alt' => get_bloginfo('title')] ) );
	}
}

/**
 * Footer heading (theme mod)
 *
 * @return void
 */
function rhd_zeitgeist_footer_heading() {
	if ( $text = get_theme_mod( 'footer_heading_text') ) {
		printf( '<div class="footer-heading-text"><h4>%s</h4></div>', $text );
	}
}