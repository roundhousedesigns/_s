	...
	function rhd_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged with %1$s', 'rhd' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
...
if ( ! function_exists( 'rhd_post_thumbnail' ) ):
	 ...
	 *
	 * @param boolean $link (default: true) Link the thumbnail to the post.
	 * @param string $size (default: 'thumbnail') The thumbnail size to request
	 * @return
	 */
	function rhd_post_thumbnail( $link = true, $size = 'thumbnail' ) {
...
function rhd_header_search_form( $button_text = 'Search', $placeholder = 'Search ...' ) {
	$form = '<form role="search" method="get" class="search-form header-search-form" action="' . home_url( '/' ) . '" >
